...
impl Cpu {
    ...
    pub fn handle_interrupts(&mut self, mmu: &mut Mmu) -> u8 {
        ...
        let ie_now = mmu.read8(0xFFFF) & 0x1F;
        let iflag_now = mmu.read8(0xFF0F) & 0x1F;
        let pending_now = ie_now & iflag_now;
        ...
        let final_i = pending_now.trailing_zeros() as u8;

        let pc_low = self.program_counter as u8;
        self.stack_pointer = self.stack_pointer.wrapping_sub(1);
        mmu.write8(self.stack_pointer, pc_low);

        self.program_counter = match final_i {
            0 => 0x40, // V-Blank
            1 => 0x48, // LCD STAT
            2 => 0x50, // Timer
            3 => 0x58, // Serial
        ...
        let new_if = (iflag & !(1 << final_i)) & 0x1F;
